...
// $Id: orders_phase6.js 186 2014-01-20 20:06:54Z paul $
function ordersInit() {
    function resChange() {
        // Check numbers are being used
        $('input[type="number"]').each(function() {if (isNaN($(this).val())) $(this).val('0');});

        // Check maximum amount has not been exceeded for buying
        var max = $('#cash').val();
        while ($('#Amount').val() * $('#Price').val() > max ) $('#Amount').val( $('#Amount').val()-1 )
    ...
    $('#Resource').change(function() {
        ...
        if ($(this).val()=='pass') {
            ...
            $('#processOrders').val('Buy').attr('disabled','disabled');
            $('#theBlind').slideDown();
        } else {
