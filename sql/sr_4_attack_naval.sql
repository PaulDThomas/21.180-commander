use asupcouk_asup;
Drop procedure if exists sr_4_attack_naval;
/*!50003 SET character_set_client  = latin1 */ ;
/*!50003 SET character_set_results = latin1 */ ;
/*!50003 SET collation_connection  = latin1_swedish_ci */ ;

-- ALTER TABLE `asupcouk_asup`.`sp_game` 
-- CHANGE COLUMN `boomers` `boomers` CHAR(1) NULL DEFAULT 'Y' ,
-- CHANGE COLUMN `boomer_tech_level` `boomer_tech_level` INT(11) NULL DEFAULT '1' ;
...
-- $Id: sr_4_attack_naval.sql 242 2014-07-13 13:48:48Z paul $
DECLARE proc_name TEXT Default "SR_4_ATTACK_NAVAL";
...
IF Upper(sr_powername) not in (Select Upper(powername) From sp_resource r Where gameno=sr_gameno and dead='N') THEN
    ...
    LEAVE sproc;
END IF;
Select userno, minerals, oil, grain, water_tech, strategic_tech
...
IF sr_major > sr_major_from_before
   or sr_minor > sr_minor_from_before THEN
    Insert Into sp_old_orders (gameno, userno, turnno, phaseno, ordername, order_code)
     Values (sr_gameno, sr_userno, sr_turnno, sr_phaseno, proc_name
...
Select Ceil(sail_nm*sr_minor*distance+sail_jm*sr_major*distance)
       ,Ceil(sail_no*sr_minor*distance+sail_jo*sr_major*distance)
       ,Ceil(sail_ng*sr_minor*distance+sail_jg*sr_major*distance)
    ...
    Insert Into sp_old_orders (gameno, userno, turnno, phaseno, ordername, order_code)
     Values (sr_gameno, sr_userno, sr_turnno, sr_phaseno, proc_name
            ,Concat("<FAIL><Reason>Not enough resource available to move and attack</Reason>"
                   ...
                   ,sf_fxml("Boomers",sr_major)
                   ,sf_fxml("Navies",sr_minor)
...
Insert Into sp_message_queue (gameno, userno, message)
 Values (sr_gameno, sr_userno
        ,Concat("You have attacked ",sr_terrname_to
               ," using ",sf_format_troops("SEA", sr_major, sr_minor)
...
Insert Into sp_message_queue (gameno, userno, message)
 Values (sr_gameno, 0
        ,Concat(sr_powername
               ...
               ," with navies from ",sr_terrname_from
               ,"."));

-- Add redeploys and retaliations
Call sr_4_attack_set(sr_gameno, "Naval", sr_def_powername, sr_terrname_to, sr_terrname_from);
-- FIGHT!
Set sr_attack_array = Concat(sf_fxml('MaxRounds',999),sf_fxml('AttackBoomers',sr_major),sf_fxml('AttackNavies',sr_minor),sf_fxml('AttackMajor',sr_att_major));
...
Insert Into sp_message_queue (gameno, userno, message)
 Values (sr_gameno, sr_userno
        ,Concat(Case 
               ...
               ,sf_format_troops("SEA",sr_major_to_after,sr_minor_to_after)
               ,". "
               )
        );
-- Attacked message
